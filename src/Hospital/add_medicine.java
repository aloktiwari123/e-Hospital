...
public class add_medicine implements ActionListener {
	...
	add_medicine()
	{
		...
		ImageIcon i1 = new ImageIcon(ClassLoader.getSystemResource("icon/medicine.jpg"));
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource()==b1)
		{
			try{
                conn cc = new conn();
                String q = "insert into medicine values('"+s1+"','"+s2+"','"+s3+"','"+s4+"','"+s5+"')";
                cc.st.executeUpdate(q);
                JOptionPane.showMessageDialog(null,"Details Successfully Inserted");
                frame.setVisible(false); // close current frame.
                new add_medicine(); // open details page
            }catch(Exception ee){
