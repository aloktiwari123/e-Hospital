...
class add_laboratory implements ActionListener {
	...
	add_laboratory()
	{
		frame = new JFrame("Add Laboratory");
		...
		l3 = new JLabel("Specification");
		l3.setBounds(100,390,150,30);
		l3.setFont(new Font("serif",Font.BOLD,20));
		frame.add(l3);
		
		l4 = new JLabel("Equipments");
		l4.setBounds(100,440,150,30);
		...
		ImageIcon i1 = new ImageIcon(ClassLoader.getSystemResource("icon/lab.jpg"));
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource()==b1)
		{
			try{
                conn cc = new conn();
                String q = "insert into laboratory values('"+s1+"','"+s2+"','"+s3+"','"+s4+"')";
                cc.st.executeUpdate(q);
                JOptionPane.showMessageDialog(null,"Details Successfully Inserted");
                frame.setVisible(false); // close current frame.
                new add_laboratory(); // open details page
            }catch(Exception ee){
