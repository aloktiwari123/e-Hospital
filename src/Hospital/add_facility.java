...
public class add_facility implements ActionListener {
	JFrame frame;
	...
	add_facility()
	{
		...
		ImageIcon i1 = new ImageIcon(ClassLoader.getSystemResource("icon/fac.jpg"));
		Image i2 = i1.getImage().getScaledInstance(500,180, Image.SCALE_DEFAULT); // resize our image size.
		ImageIcon i3 = new ImageIcon(i2);  // convert image into imageicon
		l6 =  new JLabel(i3); // set imageicon on label
		l6.setBounds(20,20,500,180);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource()==b1)
		{
			try{
                conn cc = new conn();
                String q = "insert into facility values('"+s1+"','"+s2+"','"+s3+"','"+s4+"','"+s5+"')";
                cc.st.executeUpdate(q);
                JOptionPane.showMessageDialog(null,"Details Successfully Inserted");
                frame.setVisible(false); // close current frame.
                new add_facility(); // open details page
            }catch(Exception ee){
